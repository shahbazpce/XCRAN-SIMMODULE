...
void ONU::handleMessage(cMessage *msg)
{
    ...
    if(strcmp(msg->getName(),"BWmap")==0)
    {
        ...
        if(id==onu_id)
        {
            ...
            grnt_sz+=size;
            EV<<"Granted window size from OLT= "<<size<<" for ONU: "<<id<<endl;
            EV<<"Current queue size of ONU "<<onu_id<<"= "<<data_size+temp_data_size<<endl;
            wait_time=msg->par("start_time").longValue();
            EV<<"Waiting time of ONU "<<id<<" before transmission: "<<wait_time<<endl;
            if(flag==true)
            {
                counter+=1;
    ...
    if(strcmp(msg->getName(),"Generate_report")==0)
    {
        ...
        EV<<"Current queue size of ONU "<<onu_id<<"= "<<Q_size<<endl;
        cMessage *data=new cMessage("Report");
        data->addPar("Alloc_ID");
        data->par("Alloc_ID").setLongValue(onu_id);
